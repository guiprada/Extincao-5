	...
	elseif this_type == "number" then
		for i = 1, inputs, 1 do
			...
			if not INITIALIZE_TO_VALUE then
				value = qpd_random.random() * qpd_random.choose(1, -1)
			end
			o[i] = value
		end
		n_inputs = inputs
...
function _Neuron:update(inputs)
	...
	if this_type == "table" and inputs.get_outputs then
		...
		self._output = sum
	elseif this_type == "number" then
		self._output = self[1] * inputs
	else
		print("[ERROR] - _Neuron:update() - Received a bogus input:", inputs)
	end
	self._activation_value = self.activation_function(self._output, self.activation_function_parameters or self.bias)
end
...
function _Neuron:adjust_weights(index_in_layer, inputs, output_layer, output_errors, outputs)
	...
	local output = self._activation_value
...
function _NeuronLayer:get_outputs()
	...
	local outputs = {}
	for i = 1, #self do
		outputs[i] = self[i]._activation_value
	end
...
local function mutate_factor(mutate_percentage)
	local sign = qpd_random.choose(1, -1)
	local mutate_factor = mutate_percentage + 1
	return sign * mutate_factor
end
		...
		for j = 1, #layer do
			...
			for k = 1, #layer[j] do
				if crossover and j >= crossover_layer and k >= crossover_neuron then
					-- inputs[k] = qpd_random.choose(mom[i][j][k], dad[i][j][k]) * (qpd_random.toss(mutate_chance) and qpd_random.choose(-mutate_percentage, mutate_percentage) or 1) -- old wrong logic

					inputs[k] = dad[i][j][k] * (qpd_random.toss(mutate_chance) and mutate_factor(mutate_percentage) or 1)

					-- inputs[k] = qpd_random.choose(mom[i][j][k], dad[i][j][k]) * (qpd_random.toss(mutate_chance) and mutate_factor(mutate_percentage)  or 1)
					-- inputs[k] = qpd_random.choose(mom[i][j][k], dad[i][j][k], (mom[i][j][k] + dad[i][j][k]) /2) * (qpd_random.toss(mutate_chance) and mutate_factor(mutate_percentage) or 1)
				else
					inputs[k] = mom[i][j][k] * (qpd_random.toss(mutate_chance) and mutate_factor(mutate_percentage) or 1)
				end
			end

			local bias
			if crossover and j >= crossover_layer then
				-- bias = qpd_random.choose(mom[i][j].bias, dad[i][j].bias) * (qpd_random.toss(mutate_chance) and qpd_random.choose(-mutate_percentage, mutate_percentage) or 1) -- old wrong logic

				bias = dad[i][j].bias * (qpd_random.toss(mutate_chance) and mutate_factor(mutate_percentage) or 1)

				-- bias = qpd_random.choose(mom[i][j].bias, dad[i][j].bias) * (qpd_random.toss(mutate_chance) and mutate_factor(mutate_percentage) or 1)
				-- bias = qpd_random.choose(mom[i][j].bias, dad[i][j].bias, (mom[i][j].bias + dad[i][j].bias) /2) * (qpd_random.toss(mutate_chance) and mutate_factor(mutate_percentage) or 1)
			else
				bias = mom[i][j].bias* (qpd_random.toss(mutate_chance) and mutate_factor(mutate_percentage) or 1)
			end
