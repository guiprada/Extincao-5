...
function GridActor:update(dt, speed)
	-- Hard limit is strictly < tilesize/2; we budget tilesize*4/9 (~0.444).
	-- Use 1e-9 tolerance for IEEE 754 rounding (epsilon ~1e-13).
	if speed*dt > GridActor._tilesize * 4/9 + 1e-9 then
		print(string.format("physics sanity check failed: speed*dt=%.6f > tilesize*4/9=%.6f  (speed=%.2f dt=%.8f tilesize=%d)",
			speed*dt, GridActor._tilesize * 4/9, speed, dt, GridActor._tilesize))
	end
