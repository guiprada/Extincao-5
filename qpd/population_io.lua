...
function pop_io.strategy_name(conf)
	...
	if conf.ghost_class and conf.ghost_class ~= "" and conf.ghost_class ~= "Ghost" then
		table.insert(parts, tostring(conf.ghost_class))
	end
	if conf.ghost_fitness_mode and conf.ghost_fitness_mode ~= "" then
		table.insert(parts, tostring(conf.ghost_fitness_mode))
	end
